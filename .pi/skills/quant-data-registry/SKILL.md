...
description: Use this skill to discover SignalFoundry local database coverage, market data capabilities, and the right backend endpoint before fetching financial data.
---

# SignalFoundry 数据源注册表

先查询 SignalFoundry 本地数据注册表和 TimescaleDB 覆盖，再选择具体数据能力。不要凭记忆猜接口，也不要在本地库已有数据时优先调用外部历史接口。
...
SignalFoundry 的行情链路以 PostgreSQL + TimescaleDB 为事实库。首页对话、生成工作空间、策略平台和可视化任务遇到股票、指数、ETF 或历史分析时，默认路线是：
