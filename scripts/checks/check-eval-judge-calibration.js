...
const configuredInput = argument(
  '--input',
  process.env.SIGNALFOUNDRY_EVAL_JUDGE_CALIBRATION_PATH || 'benchmarks/signalfoundry/judge-calibration.contract.json',
);
...
const requireProduction = process.env.SIGNALFOUNDRY_REQUIRE_PRODUCTION_JUDGE_CALIBRATION === '1';
...
const result = evaluateEvalJudgeCalibration({
  ...
  requireIndependent: process.env.SIGNALFOUNDRY_REQUIRE_INDEPENDENT_JUDGE === '1' || dataset.datasetKind === 'human_blind_calibration',
});
console.log(
