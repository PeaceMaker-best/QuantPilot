...
function createModelPortProvider(apiKey: string, baseUrl: string): OpenAICompatibleProvider {
  return new OpenAICompatibleProvider({
    ...
    apiKey,
    baseUrl,
    headers: { 'X-Client-App': 'SignalFoundry-Long-Term-Integration-Check/1' },
    maxRetries: 1,
...
async function checkQwen() {
  ...
  assert(llm.profileId === LOCAL_QWEN_MODEL_ID, 'SignalFoundry default LLM profile is not local Qwen.');
...
async function checkMemoryReadOnly() {
  ...
  assert(config.enabled, 'SignalFoundry personal memory integration is disabled.');
...
async function checkMemoryClosedLoop(params: {
  ...
  try {
    ...
    const preferenceInput = {
      ...
      evidenceText: 'SignalFoundry 三方长期集成的隔离合成验收偏好',
      confidence: 0.99,
