...
const baseUrl = (process.env.SIGNALFOUNDRY_WEB_URL || 'http://localhost:3000').replace(/\/+$/, '');
...
const routes = [
  { id: 'home', path: '/', expected: 'SignalFoundry' },
  ...
  { id: 'skills', path: '/skills', expected: 'SIGNALFOUNDRY SKILLS MARKET' },
...
async function discoverProjectRoute(request) {
  try {
    ...
    return id ? { id: 'chat', path: `/${id}/chat?visualCheck=1`, expected: projects[0]?.name || 'SignalFoundry' } : null;
  } catch {
    return null;
  }
}
...
async function inspectRoute(browser, storageState, route, profile) {
  ...
  await context.addInitScript((theme) => {
    localStorage.setItem('signalfoundry-color-mode', theme);
  }, profile.theme);
