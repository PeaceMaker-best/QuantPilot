...
const benchmarkRoot = path.resolve('benchmarks/signalfoundry');
...
function safeBenchmarkPath(relativePath, label) {
  ...
  if (resolved !== benchmarkRoot && !resolved.startsWith(`${benchmarkRoot}${path.sep}`)) {
    throw new Error(`${label} 必须位于 benchmarks/signalfoundry 内`);
  }
  return resolved;
}
...
function readExternalCases(definition, problems) {
  ...
  const requiredByRelease = definition.visibility === 'hidden' && process.env.SIGNALFOUNDRY_REQUIRE_HIDDEN_EVAL === '1';
  if (!configuredPath) {
