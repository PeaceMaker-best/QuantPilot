...
assertIncludes('services/market-data/src/signalfoundry_market_data/api.py', [
...
const layerDocs = {
  'services/market-data/src/signalfoundry_market_data/routers/README.md': [
    'Controller',
    'FastAPI',
    'services/',
  ],
  'services/market-data/src/signalfoundry_market_data/services/README.md': [
    'Use Case',
    'cache-aside',
    'provider fallback',
  ],
  'services/market-data/src/signalfoundry_market_data/repositories/README.md': [
    'Repository',
    'TimescaleDB',
    'PostgreSQL',
  ],
  'services/market-data/src/signalfoundry_market_data/analytics/README.md': [
    'ClickHouse',
    'TimescaleDB',
    'degraded',
  ],
...
assertIncludes('services/market-data/src/signalfoundry_market_data/routers/analytics.py', [
  'APIRouter',
  'HTTPException',
  'sync_clickhouse_analytics',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/routers/registry.py', [
  'APIRouter',
  'build_data_registry',
  '/registry',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/routers/foundation.py', [
  'APIRouter',
  'get_foundation_status',
  'scan_data_quality',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/routers/provider_candidates.py', [
  'APIRouter',
  'probe_provider_candidates',
  'CandidateProviderNotFoundError',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/routers/backtests.py', [
  'APIRouter',
  'get_ma_crossover_backtest',
  'strategy_backtest_parameters',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/routers/events.py', [
  'APIRouter',
  'get_announcements',
  'get_dividend_events',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/routers/fundamentals.py', [
  'APIRouter',
  'get_financial_reports',
  'get_fundamental_indicators',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/routers/indicators.py', [
  'APIRouter',
  'get_technical_indicators',
  'HistoricalKlineProvider',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/routers/ingestion.py', [
  'APIRouter',
  'get_market_data_ingestion_jobs',
  'control_market_data_ingestion_job',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/routers/quotes.py', [
  'APIRouter',
  'resolve_symbol',
  'get_history_quote',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/routers/research.py', [
  'APIRouter',
  'get_research_universes',
  'add_research_universe_member',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/services/analytics.py', [
  'get_clickhouse_analytics_health',
  'initialize_clickhouse_analytics',
  'sync_clickhouse_analytics',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/services/backtests.py', [
  'build_ma_crossover_backtest',
  'build_strategy_backtest',
  'read_cached_response',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/services/events.py', [
  'AnnouncementProvider',
  'DividendEventProvider',
  'read_cached_response',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/services/fundamentals.py', [
  'FinancialReportProvider',
  'build_fundamental_indicators',
  'read_cached_response',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/services/indicators.py', [
  'HistoricalKlineProvider',
  'build_technical_indicators',
  'read_cached_response',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/services/ingestion_jobs.py', [
  'list_ingestion_jobs',
  'control_ingestion_job',
  'IngestionJobControlResponse',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/services/quotes.py', [
  'SymbolResolverProvider',
  'RealtimeQuoteProvider',
  'intraday_redis_cache_key',
  'read_cached_response',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/services/research.py', [
  'ResearchUniverseProvider',
  'list_research_universes',
  'resolve_research_security',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/repositories/research.py', [
  'add_security_to_universe',
  'list_research_universes',
  '__all__',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/repositories/foundation.py', [
  'list_foundation_components',
  'run_data_quality_scan',
  '__all__',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/repositories/ingestion.py', [
  'list_ingestion_jobs',
  'update_ingestion_job_progress',
  '__all__',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/repositories/analytics.py', [
  'sync_clickhouse_daily_bars',
  '__all__',
]);

const backendPythonFiles = listFilesRecursive('services/market-data/src/signalfoundry_market_data')
  .filter((file) => file.endsWith('.py'));
for (const file of backendPythonFiles) {
  const source = read(file);
  if (
    source.includes('from signalfoundry_market_data.database import') ||
    source.includes('import signalfoundry_market_data.database')
  ) {
    fail(`${file} must depend on database_core.py or repositories/*; database.py was removed`);
  }
}
if (exists('services/market-data/src/signalfoundry_market_data/database.py')) {
  fail('database.py is a removed compatibility facade and must not be restored');
}
assertIncludes('services/market-data/src/signalfoundry_market_data/services/caching.py', [
  'read_cached_response',
  'cache_response',
  'MarketDataCache',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/services/foundation.py', [
  'list_foundation_components',
  'list_factor_definitions',
  'list_trading_calendar_days',
  'run_data_quality_scan',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/services/provider_candidates.py', [
  'CANDIDATE_PROVIDERS',
  'get_candidate_provider',
  'probe_candidate_provider',
]);
assertIncludes('services/market-data/src/signalfoundry_market_data/services/registry.py', [
  'ProviderRegistryTtls',
  'build_data_providers',
  'DataProviderInfo',
]);

const apiSource = assertFile('services/market-data/src/signalfoundry_market_data/api.py');
...
const providerBase = assertFile('services/market-data/src/signalfoundry_market_data/providers/base.py');
...
const apiLines = lineCount('services/market-data/src/signalfoundry_market_data/api.py');
if (apiLines > 1800) {
