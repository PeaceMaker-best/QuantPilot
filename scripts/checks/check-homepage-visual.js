...
async function inspectProfile(browser, storageState, profile) {
  ...
  try {
    ...
    const metrics = await page.evaluate(({ expectedTheme, touch }) => {
      ...
      const stableFallback = resources.filter((entry) => entry.name.includes('/generated/signalfoundry-tailwind.css'));
    ...
    if (metrics.stableFallbackCount > 0 && process.env.SIGNALFOUNDRY_STABLE_CSS_FALLBACK !== '1') {
      problems.push(`${profile.id}: 默认运行时仍加载重复的 signalfoundry-tailwind.css`);
    }
