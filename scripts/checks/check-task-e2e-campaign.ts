...
const baseUrl = (process.env.SIGNALFOUNDRY_TASK_E2E_BASE_URL ?? 'http://127.0.0.1:3000').replace(/\/+$/u, '');
...
async function authenticate(context: BrowserContext): Promise<void> {
  const login = process.env.SIGNALFOUNDRY_TASK_E2E_ADMIN_LOGIN?.trim() || 'admin';
  const password = process.env.SIGNALFOUNDRY_TASK_E2E_ADMIN_PASSWORD?.trim() || 'admin';
...
async function main(): Promise<void> {
  ...
  const timeoutMs = integerOption('timeout-ms',
    Number.parseInt(process.env.SIGNALFOUNDRY_TASK_E2E_TIMEOUT_MS ?? '1200000', 10),
    60_000,
    3_600_000,
  );
