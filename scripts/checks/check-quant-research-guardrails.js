...
async function main() {
  ...
  try {
    ...
    assert(/QuantScope 多标的对比/.test(page), 'generated page should use research comparison wording');
    ...
    assert(!/QuantScope 选股分析|<strong>stock-selection<\/strong>|模板组件：|候选数量|候选视图|120 日收益|<dt>120 日<\/dt>/.test(page), 'generated page should not expose legacy selection wording');
