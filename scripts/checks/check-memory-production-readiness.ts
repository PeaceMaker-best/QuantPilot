...
function requireProbeSubject(): string {
  const value = process.env.SIGNALFOUNDRY_MEMORY_PRODUCTION_PROBE_SUBJECT_ID?.trim();
  if (!value || value.length > 128) {
    throw new Error('SIGNALFOUNDRY_MEMORY_PRODUCTION_PROBE_SUBJECT_ID must name an authorized non-human probe subject.');
  }
  return value;
}
...
async function main(): Promise<void> {
  ...
  const requestId = `signalfoundry-memory-production-${Date.now()}`;
