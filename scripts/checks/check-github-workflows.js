...
if (!/SIGNALFOUNDRY_AUTH_ADMIN_EMAIL:\s*auth-smoke-admin@signalfoundry\.local/.test(quality)) {
...
if (
  !/SIGNALFOUNDRY_GENERATED_SANDBOX:\s*0/.test(quality) ||
  !/SIGNALFOUNDRY_ALLOW_UNSANDBOXED_GENERATED_CODE:\s*1/.test(quality)
) {
...
const nightlyContracts = [
  ...
  ['hosted live evaluation selects the remote DeepSeek model', /SIGNALFOUNDRY_EVAL_MODEL:\s*deepseek-v4-flash/],
...
if (!/SIGNALFOUNDRY_RELEASE_EVIDENCE_MODEL:\s*deepseek-v4-flash/.test(release)) {
