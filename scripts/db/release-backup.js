...
async function main() {
  ...
  const backupRoot = process.env.SIGNALFOUNDRY_BACKUP_ROOT || path.join(root, 'backups');
  const output = path.resolve(argument('--output', path.join(backupRoot, `signalfoundry-${timestamp}`)));
