...
function repository() {
  const repo = mkdtempSync(path.join(tmpdir(), 'signalfoundry-release-skill-'));
  roots.push(repo);
  git(repo, 'init', '--quiet');
...
function checkTarget(overrides = {}) {
  return spawnSync(process.execPath, [targetChecker], {
    ...
    env: {
      ...
      SIGNALFOUNDRY_RELEASE_HOST: 'deployer@signalfoundry-prod',
      SIGNALFOUNDRY_PUBLIC_URL: 'https://signalfoundry.example.com',
      SIGNALFOUNDRY_RELEASE_ROOT: '/opt/signalfoundry',
      SIGNALFOUNDRY_RELEASE_ENV_FILE: '/etc/signalfoundry/signalfoundry.env',
      SIGNALFOUNDRY_BACKUP_ROOT: '/var/backups/signalfoundry',
      ...overrides,
    },
  });
}
