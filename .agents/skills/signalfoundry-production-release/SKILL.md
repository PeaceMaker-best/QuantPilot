...
Use repository-relative paths from the SignalFoundry root. Do not copy secrets into
...
```bash
node .agents/skills/signalfoundry-production-release/scripts/self-test.mjs
uv run --with pyyaml -- python \
  "${CODEX_HOME:-$HOME/.codex}/skills/.system/skill-creator/scripts/quick_validate.py" \
  .agents/skills/signalfoundry-production-release
```
