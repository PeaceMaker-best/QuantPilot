...
| --- | --- | --- |
| `SIGNALFOUNDRY_RELEASE_HOST` | Approved SSH host or configured SSH alias | Required |
| `SIGNALFOUNDRY_PUBLIC_URL` | HTTPS URL used for public smoke tests | Required |
| `SIGNALFOUNDRY_RELEASE_ROOT` | Root containing immutable releases and `current` | `/opt/signalfoundry` |
| `SIGNALFOUNDRY_RELEASE_ENV_FILE` | Root-only production environment file | `/etc/signalfoundry/signalfoundry.env` |
| `SIGNALFOUNDRY_BACKUP_ROOT` | Production backup root | `/var/backups/signalfoundry` |
