...
export default function PersonalMemoryClient() {
      ...
      const response = await fetch(
        `/api/account/memory/preferences/${encodeURIComponent(preference.recordId)}/corrections`,
        {
          ...
          body: JSON.stringify({
            ...
            value,
            evidenceText: `用户在 SignalFoundry 账号记忆中心明确修正：${value}`,
            reason,
  ...
  return (
    <AccountPageShell
      title="用户记忆"
      subtitle="决定外部记忆是否可以影响未来的 SignalFoundry 任务，并查看当前保存的偏好。"
    ...
    >
      ...
      {loading && !state ? (
        ...
        <>
              ...
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">
                    启用后，SignalFoundry 才会在新任务开始前召回你的分析、输出和研究偏好。关闭状态下不会向 Memory 发起任务召回。
                  </p>
                </div>
              </div>
              <Button
          ...
          <section className="rounded-2xl border bg-card p-5 shadow-sm sm:p-6">
            <div className="flex items-center gap-3"><Database className="h-5 w-5 text-primary" /><div><h2 className="font-semibold">当前偏好</h2><p className="text-sm text-muted-foreground">仅展示属于当前登录账号、且标记为 SignalFoundry 的偏好。</p></div></div>
            <div className="mt-5 grid gap-3">
              ...
              {state.preferences?.length === 0 ? <p className="rounded-xl border border-dashed p-6 text-center text-sm leading-6 text-muted-foreground">还没有 SignalFoundry 用户偏好。进入任一项目对话，在输入框下方点击“记住偏好”并明确确认后，偏好才会写入。</p> : null}
