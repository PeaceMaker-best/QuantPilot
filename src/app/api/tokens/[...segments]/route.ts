...
function internalTokenApiEnabled(): boolean {
  return process.env.SIGNALFOUNDRY_ENABLE_INTERNAL_TOKEN_API === '1';
}

function internalTokenApiAuthorized(request: NextRequest): boolean {
  const expected = process.env.SIGNALFOUNDRY_INTERNAL_API_TOKEN;
  const provided = request.headers.get('x-signalfoundry-internal-token');
