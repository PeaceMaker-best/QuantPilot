...
function resolveDashboardCapability(
  ...
  if (!KNOWN_TEMPLATE_IDS.has(templateId)) {
    throw new PiAgentToolError(
      ...
      `No trusted SignalFoundry renderer is registered for template ${templateId}.`,
  ...
  if (!capability) {
    throw new PiAgentToolError(
      ...
      `No trusted SignalFoundry renderer capability is registered for ${templateId}/${variantId}.`,
      { templateId, variantId },
    );
  }
