...
function resolveDashboardCapability(
  ...
  if (!KNOWN_TEMPLATE_IDS.has(templateId)) {
    throw new PiAgentToolError(
      ...
      `No trusted QuantScope renderer is registered for template ${templateId}.`,
  ...
  if (!capability) {
    throw new PiAgentToolError(
      ...
      `No trusted QuantScope renderer capability is registered for ${templateId}/${variantId}.`,
      { templateId, variantId },
    );
  }
