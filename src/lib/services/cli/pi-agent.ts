/** SignalFoundry integration for the upstream PI Agent runtime. */
...
async function buildBoundedHistory(
  ...
  const messages = (await getRecentChatMessagesByProjectId(projectId, HISTORY_FETCH_LIMIT))
    .filter((message) => {
      ...
      return metadata?.isSignalFoundryPipelineStep !== true &&
        metadata?.toolName !== 'SignalFoundry 自动验证' &&
...
/**
 * Execute one upstream PI Agent run. SignalFoundry owns planning/validation and
...
async function executePiAgentPhase(
  ...
  try {
    ...
    const [skillBundle, taskPrompt, history] = await raceWithAbort(Promise.all([
      ...
      buildSignalFoundryTaskPrompt(instruction, workspace, {
        runPlan,
        platformPrepared,
        preparedIntent,
        phase: skillPhase,
    ...
    const systemPrompt = buildSignalFoundrySystemPrompt({
      phase: skillPhase,
      preparedIntent,
...
Keep the App Router, TypeScript, package setup, local CSS, market proxy, platform-prefetched run plan, final data, evidence, and dashboard data binding. Do not recreate the project or reset package.json. At 390x844 the first viewport must show the instrument, price, at least two real metrics, and the main visualization body. At 1440x900 keep the primary visualization above the fold. SignalFoundry will run build, preview, and validation after submit_result.`;
