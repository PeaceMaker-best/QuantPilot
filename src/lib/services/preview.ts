...
function resolveMarketDataTcpTarget(): { host: string; port: number } {
  const configured = process.env.SIGNALFOUNDRY_MARKET_API_URL?.trim()
    || 'http://127.0.0.1:8000';
...
async function isPidWithinProject(pid: number, projectPath: string): Promise<boolean> {
  ...
  try {
    ...
    // /tmp/signalfoundry-generated-sandbox.XYZ/<absolute project path>.
    ...
    if (!path.basename(processRoot).startsWith('signalfoundry-generated-sandbox.')) {
      return false;
    }
...
export class PreviewManager {
  ...
  private async startInternal(
    ...
    if (usesGeneratedProjectNetworkNamespace()) {
      ...
      env.SIGNALFOUNDRY_SANDBOX_PREVIEW_SOCKET = previewSocketPath;
      env.SIGNALFOUNDRY_SANDBOX_PREVIEW_PORT = String(effectivePort);
      env.SIGNALFOUNDRY_SANDBOX_MARKET_SOCKET = marketSocketPath;
      env.SIGNALFOUNDRY_SANDBOX_MARKET_PORT = '8000';
