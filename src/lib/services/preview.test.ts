...
vi.mock('@/lib/security/generated-project-sandbox', () => ({
  ...
  ) => ({ ...overrides, SIGNALFOUNDRY_WORKSPACE_ROOT: projectPath }),
...
describe('PreviewManager start concurrency', () => {
  beforeEach(async () => {
    ...
    mkdirSync('/tmp/signalfoundry-preview-test/.next', { recursive: true });
    ...
    mocks.getProjectById.mockResolvedValue({
      id: 'project-preview',
      previewPort: null,
      repoPath: '/tmp/signalfoundry-preview-test',
    });
  ...
  it('coalesces concurrent starts into one process and one ready result', async () => {
    ...
    expect(previewEnv.SIGNALFOUNDRY_SANDBOX_PREVIEW_SOCKET).toMatch(
      /^\/tmp\/qp-preview\/[a-f0-9]{20}\/p\.sock$/,
    );
    expect(previewEnv.SIGNALFOUNDRY_SANDBOX_MARKET_SOCKET).toMatch(
  ...
  it('lets a trusted unsandboxed preview bind the selected TCP port directly', async () => {
    ...
    const previewEnv = mocks.spawn.mock.calls[0]?.[2]?.env as NodeJS.ProcessEnv;
    expect(previewEnv.SIGNALFOUNDRY_SANDBOX_PREVIEW_SOCKET).toBeUndefined();
    expect(previewEnv.SIGNALFOUNDRY_SANDBOX_MARKET_SOCKET).toBeUndefined();
  ...
  it('installs generated dependencies without host lifecycle scripts', async () => {
    ...
    expect(mocks.spawn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ cwd: '/tmp/signalfoundry-preview-test' }),
    );
  });
  ...
  it('discovers an existing project preview with one listener-table snapshot', async () => {
    ...
    fsMocks.readlink.mockImplementation(async (linkPath: unknown) => {
      if (String(linkPath) === '/proc/7173/cwd') {
        return '/tmp/signalfoundry-preview-test';
      }
  ...
  it('adopts a project listener whose cwd is inside the generated chroot', async () => {
    ...
    fsMocks.readlink.mockImplementation(async (linkPath: unknown) => {
      if (String(linkPath) === '/proc/7173/cwd') {
        return '/tmp/signalfoundry-generated-sandbox.ABC/tmp/signalfoundry-preview-test';
      }
      if (String(linkPath) === '/proc/7173/root') {
        return '/tmp/signalfoundry-generated-sandbox.ABC';
      }
      throw missingFile();
    });
