...
const ACTIVE_REQUEST_STALE_MS =
  Number.parseInt(process.env.SIGNALFOUNDRY_ACTIVE_REQUEST_STALE_MS ?? '', 10) || 30 * 60 * 1000;
const ACTIVE_REQUEST_ORPHAN_MS =
  Number.parseInt(process.env.SIGNALFOUNDRY_ACTIVE_REQUEST_ORPHAN_MS ?? '', 10) || 8 * 60 * 1000;
...
const globalForActiveRequests = globalThis as unknown as {
  __signalfoundry_active_request_runtime__?: Set<string>;
};
const activeRuntimeRequests =
  globalForActiveRequests.__signalfoundry_active_request_runtime__ ??
  (globalForActiveRequests.__signalfoundry_active_request_runtime__ = new Set<string>());
