...
export type SignalFoundryAuthMode = 'disabled' | 'local';
...
function modeEnv(value: string | undefined, fallback: SignalFoundryAuthMode): SignalFoundryAuthMode {
  const normalized = value?.trim().toLowerCase();
...
export function getProjectAuthConfig(
  ...
  if (
    ...
    throw new Error('config/auth.json does not match the locked SignalFoundry auth contract.');
  }
