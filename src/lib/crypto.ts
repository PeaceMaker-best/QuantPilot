...
function shouldRequireConfiguredKey(): boolean {
  ...
  return process.env.NODE_ENV === 'production' || process.env.SIGNALFOUNDRY_DEGRADATION_MODE === 'strict';
}
...
function resolveEncryptionKey(): string {
  ...
  if (isProductionBuildPhase()) {
    return crypto.createHash('sha256').update('signalfoundry-local-development-encryption-key').digest('hex');
  }
