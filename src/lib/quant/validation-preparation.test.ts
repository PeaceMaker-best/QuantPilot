...
beforeEach(() => {
  previousSettleTimeout = process.env.SIGNALFOUNDRY_VALIDATION_SETTLE_TIMEOUT_MS;
  process.env.SIGNALFOUNDRY_VALIDATION_SETTLE_TIMEOUT_MS = '1';
});

afterEach(async () => {
  if (previousSettleTimeout === undefined) {
    delete process.env.SIGNALFOUNDRY_VALIDATION_SETTLE_TIMEOUT_MS;
  } else {
    process.env.SIGNALFOUNDRY_VALIDATION_SETTLE_TIMEOUT_MS = previousSettleTimeout;
  }
  await Promise.all(
...
describe('quant validation preparation', () => {
  it('normalizes generated build inputs once and remains content-idempotent', async () => {
    const projectPath = await fs.mkdtemp(path.join(os.tmpdir(), 'signalfoundry-validation-prep-'));
