...
function purpose(value: string | undefined): string {
  ...
  if (!PURPOSE_PATTERN.test(normalized)) {
    throw new Error('SIGNALFOUNDRY_KNOWLEDGE_PURPOSE is invalid.');
  }
  return normalized;
}
...
function spaces(value: string | undefined): string[] {
  ...
  if (items.length === 0 || items.length > 50) {
    throw new Error('SIGNALFOUNDRY_KNOWLEDGE_SPACES must contain between 1 and 50 URI values.');
  }
  for (const item of items) {
    const parsed = new URL(item);
    if (!parsed.protocol || parsed.username || parsed.password) {
      throw new Error('SIGNALFOUNDRY_KNOWLEDGE_SPACES contains an invalid URI.');
    }
  }
...
function uriBase(value: string | undefined): string {
  const normalized = value?.trim()
    || 'https://knowledge.local/spaces/signalfoundry/projects';
  const parsed = new URL(normalized);
  if (!parsed.protocol || parsed.username || parsed.password || parsed.search || parsed.hash) {
    throw new Error('SIGNALFOUNDRY_KNOWLEDGE_PROJECT_SPACE_BASE_URL contains an invalid URI.');
  }
...
function oauthConfig(
  ...
  const tokenUrl = environment.SIGNALFOUNDRY_KNOWLEDGE_OAUTH_TOKEN_URL?.trim();
  const clientId = environment.SIGNALFOUNDRY_KNOWLEDGE_OAUTH_CLIENT_ID?.trim();
  const clientSecret = environment.SIGNALFOUNDRY_KNOWLEDGE_OAUTH_CLIENT_SECRET?.trim();
  ...
  const normalizedTokenUrl = httpBaseUrl(tokenUrl, 'SIGNALFOUNDRY_KNOWLEDGE_OAUTH_TOKEN_URL');
  ...
  const resource = environment.SIGNALFOUNDRY_KNOWLEDGE_OAUTH_RESOURCE?.trim()
    || new URL('/akep/0.1', `${apiUrl}/`).toString();
  const scope = environment.SIGNALFOUNDRY_KNOWLEDGE_OAUTH_SCOPE?.trim()
...
export function getKnowledgeIntegrationConfig(
  ...
  const enabled = !offline && flag(
    environment.SIGNALFOUNDRY_KNOWLEDGE_ENABLED,
    environment.NODE_ENV !== 'test',
  );
  const required = enabled && flag(environment.SIGNALFOUNDRY_KNOWLEDGE_REQUIRED, false);
  const apiUrl = httpBaseUrl(
    environment.SIGNALFOUNDRY_KNOWLEDGE_API_URL?.trim() || 'http://localhost:33005',
    'SIGNALFOUNDRY_KNOWLEDGE_API_URL',
  );
  ...
  const bearerToken = environment.SIGNALFOUNDRY_KNOWLEDGE_BEARER_TOKEN?.trim() || null;
  ...
  return {
    ...
    required,
    apiUrl,
    purpose: purpose(environment.SIGNALFOUNDRY_KNOWLEDGE_PURPOSE),
    spaces: spaces(environment.SIGNALFOUNDRY_KNOWLEDGE_SPACES),
    projectSpacesEnabled: flag(environment.SIGNALFOUNDRY_KNOWLEDGE_PROJECT_SPACES_ENABLED, true),
    projectSpaceBaseUrl: uriBase(environment.SIGNALFOUNDRY_KNOWLEDGE_PROJECT_SPACE_BASE_URL),
    timeoutMs: boundedInteger(environment.SIGNALFOUNDRY_KNOWLEDGE_TIMEOUT_MS, 2_000, 100, 30_000),
    maxContextCharacters: boundedInteger(
      environment.SIGNALFOUNDRY_KNOWLEDGE_MAX_CONTEXT_CHARACTERS,
      8_000,
      256,
      100_000,
    ),
