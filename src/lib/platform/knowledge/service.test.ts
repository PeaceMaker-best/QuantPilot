...
const scope = createProjectIntegrationScope({
  projectId: 'project-1',
  memory: { tenantId: 'tenant-signalfoundry' },
  knowledge: config,
});
...
describe('governed knowledge service', () => {
  it('prepares a bounded untrusted capsule and records accepted-run usage', async () => {
    ...
    expect(adapter.recordUsage).toHaveBeenCalledWith(
      ...
      expect.stringContaining('signalfoundry-knowledge-usage-request-1-'),
      'request-1',
    );
    ...
    expect(adapter.recordFeedback).toHaveBeenCalledWith(
      ...
      expect.stringContaining('signalfoundry-knowledge-feedback-request-1-'),
      'request-1',
    );
  });
