...
describe('AKEP HTTP adapter', () => {
  it('discovers a versioned same-authority contract without importing provider code', async () => {
    ...
    await expect(adapter.discover('signalfoundry-test')).resolves.toMatchObject({
      protocol: 'akep',
      versions: ['0.1'],
  ...
  it('submits privacy-bounded Mission outcome evidence against a real usage', async () => {
    const fetcher = vi.fn()
      .mockResolvedValueOnce(jsonResponse(discovery()))
      .mockResolvedValueOnce(jsonResponse({
        feedbackId: 'urn:signalfoundry:knowledge-feedback:abc',
        ...
        evaluatorVersion: {
          uri: 'urn:signalfoundry:evaluator:mission-acceptance:v1',
    ...
    await expect(adapter.recordFeedback({
      ...
      contextDigest: `sha256:${'b'.repeat(64)}`,
      evidenceRefs: ['urn:signalfoundry:mission-receipt:abc'],
