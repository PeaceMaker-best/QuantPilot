...
const BUSINESS_OUTCOME_EVALUATOR_URI = 'urn:signalfoundry:evaluator:human-business-outcome:v1';
const BUSINESS_OUTCOME_EVALUATOR_DIGEST = `sha256:${createHash('sha256')
  .update('signalfoundry-human-business-outcome-v1', 'utf8')
  .digest('hex')}`;
  ...
  try {
    ...
    for (const [spaceId, citations] of bySpace) {
      ...
      const receipt = await runtime.port.recordUsage({
        clientUsageId: `signalfoundry:${input.requestId}:${suffix}`,
        ...
        occurredAt,
      }, `signalfoundry-knowledge-usage-${input.requestId}-${suffix}`, input.requestId);
      if (
...
export async function recordGovernedKnowledgeFeedback(input: {
  ...
  try {
    ...
    for (const usage of input.usage.usageReceipts) {
      ...
      const feedbackId = `urn:signalfoundry:knowledge-feedback:${suffix}`;
      const receipt = await runtime.port.recordFeedback({
        ...
        evidenceRefs: [
          `urn:signalfoundry:mission-receipt:${stableSuffix(
            `${input.acceptedReceiptId}:${input.acceptedReceiptSha256}`,
          )}`,
          `urn:signalfoundry:business-feedback:${stableSuffix(input.eventId)}`,
        ],
        observedAt: input.observedAt ?? new Date().toISOString(),
        privacy: { rawTaskStored: false, aggregation: 'pseudonymized' },
      }, `signalfoundry-knowledge-feedback-${input.requestId}-${suffix}`, input.requestId);
      if (
