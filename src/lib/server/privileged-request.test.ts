...
const original = {
  token: process.env.SIGNALFOUNDRY_ADMIN_TOKEN,
  degradation: process.env.SIGNALFOUNDRY_DEGRADATION_MODE,
};

afterEach(() => {
  if (original.token === undefined) delete process.env.SIGNALFOUNDRY_ADMIN_TOKEN;
  else process.env.SIGNALFOUNDRY_ADMIN_TOKEN = original.token;
  if (original.degradation === undefined) delete process.env.SIGNALFOUNDRY_DEGRADATION_MODE;
  else process.env.SIGNALFOUNDRY_DEGRADATION_MODE = original.degradation;
});
...
describe('assertPrivilegedMutation', () => {
  it('allows a same-origin loopback development mutation', () => {
    delete process.env.SIGNALFOUNDRY_ADMIN_TOKEN;
  ...
  it('requires a matching token whenever one is configured', () => {
    process.env.SIGNALFOUNDRY_ADMIN_TOKEN = 'test-admin-token';
    expect(() => assertPrivilegedMutation(request())).toThrowError(/管理令牌/);
    expect(() => assertPrivilegedMutation(request(undefined, { 'x-signalfoundry-admin-token': 'test-admin-token' }))).not.toThrow();
  });

  it('fails closed for a non-loopback host without a token', () => {
    delete process.env.SIGNALFOUNDRY_ADMIN_TOKEN;
    expect(() => assertPrivilegedMutation(request('https://signalfoundry.example/api/skills'))).toThrowError(/非本机写入/);
  });
});
