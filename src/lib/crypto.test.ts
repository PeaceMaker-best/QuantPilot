...
const originalEnvironment = {
  ...
  degradationMode: process.env.SIGNALFOUNDRY_DEGRADATION_MODE,
};
...
beforeEach(() => {
  ...
  delete process.env.SIGNALFOUNDRY_DEGRADATION_MODE;
});
...
afterAll(() => {
  ...
  if (originalEnvironment.degradationMode === undefined) delete process.env.SIGNALFOUNDRY_DEGRADATION_MODE;
  else process.env.SIGNALFOUNDRY_DEGRADATION_MODE = originalEnvironment.degradationMode;
});
...
describe('crypto', () => {
  it('round-trips secrets with the authenticated v2 format', async () => {
    ...
    const encrypted = encrypt('signalfoundry-secret-量化');

    expect(isCurrentEncryptionFormat(encrypted)).toBe(true);
    expect(encrypted).not.toContain('signalfoundry-secret');
    expect(decrypt(encrypted)).toBe('signalfoundry-secret-量化');
  });
