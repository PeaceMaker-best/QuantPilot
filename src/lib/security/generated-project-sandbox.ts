...
export function buildGeneratedProjectEnv(
  ...
  const env: Record<string, string | undefined> = {
    ...
    SIGNALFOUNDRY_WORKSPACE_ROOT: path.resolve(projectPath),
  };
...
export async function wrapGeneratedProjectCommand(
  ...
  if (process.platform !== 'linux') {
    if (process.env.SIGNALFOUNDRY_ALLOW_UNSANDBOXED_GENERATED_CODE === '1') {
      return { command, args: [...args] };
    }
    throw new Error(
      'Generated project execution requires the Linux namespace sandbox. ' +
      'Set SIGNALFOUNDRY_ALLOW_UNSANDBOXED_GENERATED_CODE=1 only in an isolated development machine.',
    );
  }
  if (process.env.SIGNALFOUNDRY_GENERATED_SANDBOX === '0') {
    if (process.env.SIGNALFOUNDRY_ALLOW_UNSANDBOXED_GENERATED_CODE !== '1') {
