...
/** SDK cost fields are placeholders; SignalFoundry has no provider billing receipt here. */
 ...
 * SignalFoundry's historical inputTokens is the full provider input.
 */
...
export function usageToPi(usage: PiAgentTokenUsage): HostUsage {
  const fullInput = tokenCount(usage.inputTokens, 'SignalFoundry input usage');
  const output = tokenCount(usage.outputTokens, 'SignalFoundry output usage');
  if (tokenCount(usage.totalTokens, 'SignalFoundry total usage') !== fullInput + output) {
    throw new Error('SignalFoundry total usage does not match input plus output usage.');
  }
  let cacheRead =
    usage.cachedInputTokens === undefined
      ? undefined
      : tokenCount(usage.cachedInputTokens, 'SignalFoundry cached-input usage');
  let cacheMiss =
    usage.cacheMissInputTokens === undefined
      ? undefined
      : tokenCount(usage.cacheMissInputTokens, 'SignalFoundry cache-miss usage');
  if (cacheRead === undefined && cacheMiss === undefined) {
    cacheRead = 0;
    cacheMiss = fullInput;
  } else if (cacheRead === undefined) {
    if (cacheMiss! > fullInput) {
      throw new Error('SignalFoundry cache-miss usage cannot exceed full input usage.');
    }
    cacheRead = fullInput - cacheMiss!;
  } else if (cacheMiss === undefined) {
    if (cacheRead > fullInput) {
      throw new Error('SignalFoundry cached-input usage cannot exceed full input usage.');
    }
    cacheMiss = fullInput - cacheRead;
  }
  if (cacheRead === undefined || cacheMiss === undefined) {
    throw new Error('SignalFoundry usage normalization failed.');
  }
  if (cacheRead + cacheMiss !== fullInput) {
    throw new Error('SignalFoundry cached and cache-miss usage must equal full input usage.');
  }
  if (usage.reasoningTokens !== undefined && tokenCount(usage.reasoningTokens, 'SignalFoundry reasoning usage') > output) {
    throw new Error('SignalFoundry reasoning usage cannot exceed output usage.');
  }
