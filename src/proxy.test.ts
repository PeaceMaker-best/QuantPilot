...
const originalMode = process.env.SIGNALFOUNDRY_AUTH_MODE;
const originalSecret = process.env.SIGNALFOUNDRY_AUTH_SECRET;
...
describe('project authentication proxy', () => {
  beforeEach(() => {
    ...
    process.env.SIGNALFOUNDRY_AUTH_MODE = 'local';
    process.env.SIGNALFOUNDRY_AUTH_SECRET = 'test-secret-that-is-longer-than-thirty-two-characters';
  });

  afterEach(() => {
    if (originalMode === undefined) delete process.env.SIGNALFOUNDRY_AUTH_MODE;
    else process.env.SIGNALFOUNDRY_AUTH_MODE = originalMode;
    if (originalSecret === undefined) delete process.env.SIGNALFOUNDRY_AUTH_SECRET;
    else process.env.SIGNALFOUNDRY_AUTH_SECRET = originalSecret;
  });

  it('keeps the existing anonymous workflow when auth is disabled', async () => {
    process.env.SIGNALFOUNDRY_AUTH_MODE = 'disabled';
  ...
  it.each([
    ...
    '/icons/signalfoundry-192.png',
    '/icons/signalfoundry-512.png',
