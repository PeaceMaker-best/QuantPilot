...
from signalfoundry_market_data import clickhouse
from signalfoundry_market_data.api import create_app
from signalfoundry_market_data.clickhouse import get_clickhouse_health, is_clickhouse_enabled


@pytest.mark.anyio
async def test_clickhouse_health_is_disabled_by_default(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("SIGNALFOUNDRY_CLICKHOUSE_ENABLED", raising=False)
    ...
    assert health.database == "signalfoundry"


def test_clickhouse_health_endpoint_when_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("SIGNALFOUNDRY_CLICKHOUSE_ENABLED", "0")
...
def test_registry_exposes_clickhouse_analytics_provider() -> None:
    ...
    assert "signalfoundry-clickhouse-analytics" in provider_ids
