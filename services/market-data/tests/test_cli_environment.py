...
from signalfoundry_market_data.cli import load_market_environment
...
def test_market_environment_uses_allowlist_and_local_precedence(tmp_path, monkeypatch) -> None:
    ...
    (tmp_path / ".env.local").write_text(
        ...
        "SIGNALFOUNDRY_REDIS_REQUIRED=1\n",
        encoding="utf8",
    )
    monkeypatch.delenv("REDIS_URL", raising=False)
    monkeypatch.delenv("SIGNALFOUNDRY_REDIS_REQUIRED", raising=False)
    monkeypatch.delenv("DEEPSEEK_API_KEY", raising=False)

    load_market_environment(tmp_path)

    assert os.environ["REDIS_URL"] == "redis://local:6379/0"
    assert os.environ["SIGNALFOUNDRY_REDIS_REQUIRED"] == "1"
