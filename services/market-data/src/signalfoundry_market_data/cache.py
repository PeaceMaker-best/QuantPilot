...
from signalfoundry_market_data.contracts.common import FetchMetadata
...
class RedisJsonCache:
    ...
    def __init__(
        ...
        *,
        ...
        self.namespace = namespace or os.getenv("REDIS_NAMESPACE", "signalfoundry")
...
def default_cache_root() -> Path:
    raw_dir = os.getenv("SIGNALFOUNDRY_MARKET_CACHE_DIR")
    if raw_dir:
        return Path(raw_dir).expanduser()

    xdg_cache_home = os.getenv("XDG_CACHE_HOME")
    if xdg_cache_home:
        return Path(xdg_cache_home).expanduser() / "signalfoundry" / "market_data"
    return Path("~/.cache/signalfoundry/market_data").expanduser()


def cache_enabled_from_env() -> bool:
    raw_value = os.getenv("SIGNALFOUNDRY_MARKET_CACHE_ENABLED")
...
def redis_enabled_from_env() -> bool:
    raw_value = os.getenv("SIGNALFOUNDRY_REDIS_CACHE_ENABLED")
    if raw_value is None:
        return True
    return raw_value.strip().lower() not in {"0", "false", "no", "off", "disabled"}
