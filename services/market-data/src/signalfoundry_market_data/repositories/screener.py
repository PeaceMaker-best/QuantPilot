...
from signalfoundry_market_data.cache import RedisJsonCache, ttl_from_env
from signalfoundry_market_data.clickhouse import (
...
from signalfoundry_market_data.repositories.analytics import sync_clickhouse_daily_bars
...
async def screen_a_share_short_term_candidates(
    *,
    ...
    notes = [
        (
            "本接口通过 SignalFoundry market-data API 读取 ClickHouse 分析表；"
            "skills 不直接访问数据库。"
            if data_basis.startswith("clickhouse.")
            else (
                "本接口只通过 SignalFoundry market-data API 读取本地 TimescaleDB；"
                "skills 不直接访问数据库。"
            )
        ),
