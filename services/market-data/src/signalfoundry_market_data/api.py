...
from signalfoundry_market_data.cache import MarketDataCache, RedisJsonCache, ttl_from_env
from signalfoundry_market_data.contracts.ingestion import (
...
from signalfoundry_market_data.database_core import (
    DatabaseError,
    connect,
    normalize_fetch_symbol,
)
from signalfoundry_market_data.providers.akshare import AkShareClient, AkShareError
from signalfoundry_market_data.providers.baostock import BaoStockClient, BaoStockError
from signalfoundry_market_data.providers.eastmoney import EastMoneyClient, EastMoneyError
from signalfoundry_market_data.repositories.ingestion import (
    create_ingestion_job,
    finish_ingestion_job,
    get_history_ingestion_preflight,
    update_ingestion_job_progress,
)
from signalfoundry_market_data.repositories.universes import get_universe_fetch_targets
from signalfoundry_market_data.repositories.upserts import (
    upsert_kline_response,
    upsert_realtime_quote_snapshot,
)
from signalfoundry_market_data.routers.analytics import router as analytics_router
from signalfoundry_market_data.routers.backtests import create_backtest_router
from signalfoundry_market_data.routers.context import create_context_router
from signalfoundry_market_data.routers.events import create_events_router
from signalfoundry_market_data.routers.foundation import router as foundation_router
from signalfoundry_market_data.routers.fundamentals import create_fundamentals_router
from signalfoundry_market_data.routers.indicators import create_indicators_router
from signalfoundry_market_data.routers.ingestion import router as ingestion_router
from signalfoundry_market_data.routers.lifecycle import create_lifecycle_router
from signalfoundry_market_data.routers.provider_candidates import (
    router as provider_candidates_router,
)
from signalfoundry_market_data.routers.quotes import create_quotes_router
from signalfoundry_market_data.routers.registry import create_registry_router
from signalfoundry_market_data.routers.research import create_research_router
from signalfoundry_market_data.security import require_market_admin
from signalfoundry_market_data.services.ingestion_support import (
...
from signalfoundry_market_data.services.registry import ProviderRegistryTtls

QUOTE_CACHE_TTL_SECONDS = ttl_from_env("SIGNALFOUNDRY_QUOTE_CACHE_TTL_SECONDS", 5)
SYMBOL_CACHE_TTL_SECONDS = ttl_from_env("SIGNALFOUNDRY_SYMBOL_CACHE_TTL_SECONDS", 86400)
KLINE_CACHE_TTL_SECONDS = ttl_from_env("SIGNALFOUNDRY_KLINE_CACHE_TTL_SECONDS", 1800)
FINANCIAL_CACHE_TTL_SECONDS = ttl_from_env("SIGNALFOUNDRY_FINANCIAL_CACHE_TTL_SECONDS", 21600)
ANNOUNCEMENT_CACHE_TTL_SECONDS = ttl_from_env("SIGNALFOUNDRY_ANNOUNCEMENT_CACHE_TTL_SECONDS", 600)
SCREENER_CACHE_TTL_SECONDS = ttl_from_env("SIGNALFOUNDRY_SCREENER_CACHE_TTL_SECONDS", 60)
