...
def market_admin_token_required() -> bool:
    mode = os.getenv("SIGNALFOUNDRY_DEGRADATION_MODE", "auto").strip().lower()
...
async def require_market_admin(
    ...
    x_signalfoundry_admin_token: str | None = Header(default=None),
) -> None:
    """Protect mutating market endpoints while keeping loopback development frictionless."""
    configured = os.getenv("SIGNALFOUNDRY_MARKET_ADMIN_TOKEN", "").strip()
    if not configured:
    ...
    presented = x_signalfoundry_admin_token or _bearer_token(authorization)
