...
class ReadinessResult(TypedDict):
    ok: bool
    service: Literal["signalfoundry-market-data"]
...
async def get_market_readiness(
    *,
    ...
    strict = env.get("SIGNALFOUNDRY_DEGRADATION_MODE", "auto").strip().lower() == "strict"
    database_enabled = env_flag(env, "SIGNALFOUNDRY_DATABASE_ENABLED", True)
    redis_enabled = env_flag(env, "SIGNALFOUNDRY_REDIS_CACHE_ENABLED", True)
    definitions = [
        (
            "database",
            database_enabled,
            env_flag(env, "SIGNALFOUNDRY_DATABASE_REQUIRED", True),
            database_probe,
        ),
        (
            "redis",
            redis_enabled,
            env_flag(env, "SIGNALFOUNDRY_REDIS_REQUIRED", strict),
            redis_probe,
        ),
    ]
    components: dict[str, ReadinessComponent] = {}
    ...
    return {
        ...
        "service": "signalfoundry-market-data",
