...
投研情报中心把“观察池、数据采样、报告契约、主题洞察、推送回执”串成一条可验证链路。它参考多数据源日报项目的优点，但不把 SignalFoundry 变成外部 API Key 拼装脚本：真实行情仍从本地 market-data 读取，报告先保存为结构化对象，再由页面、推送和后续 LLM 摘要共同使用。
...
| 能力 | 当前状态 | 说明 |
| --- | --- | --- |
| 自选池订阅 | 已落库 | 默认生成 `SignalFoundry 每日核心观察池`，绑定 `a-share-sample-research-pool` 和少量核心标的 |
