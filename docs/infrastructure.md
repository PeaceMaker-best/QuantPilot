...
需要个性化记忆时，先在独立的 `evolvable-user-memory` 仓库启动 Memory API，再启动或重启 SignalFoundry Web。持久化本地模式推荐在 Memory 仓库运行：
...
当前不引入 Dubbo3 这类 Java 服务治理栈。SignalFoundry 会长期保持 Python/FastAPI + Node/Next.js 的主线，所以服务注册、配置中心和依赖发现先用更轻的方式落地：
