...
| PI Agent（执行器 `pi`） | `deepseek:deepseek-v4-flash`（日常可选） | SignalFoundry 调 ModelPort OpenAI-compatible `/chat/completions`；ModelPort 调 DeepSeek Anthropic `/v1/messages` | 分析与生成 |
