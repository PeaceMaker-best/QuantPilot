...
```bash
SIGNALFOUNDRY_DEGRADATION_MODE=offline npm run dev
```

## AetherGateway、Memory 与 AKEP 日常检查
...
成功表示 SignalFoundry 默认 Qwen profile、AetherGateway Qwen 与 DeepSeek 的模型发现/鉴权/工具流和续写、Qwen LLM Query Rewrite、Memory 契约和 readiness 均正常。DeepSeek 上游使用 AetherGateway 的 Anthropic provider。该命令会产生真实模型 Token，但不写 Memory；输出不会包含凭据或记忆正文。AKEP 的检索、Citation、Usage 和 Feedback 由下方 `check:triad-experience` 验收，不把 Model/Memory 的只读探针误写成四方全链路。
...
`production_ready=true`、`jwt/access_token` 身份边界、SignalFoundry 使用短期 token broker，并以配置的
...
它覆盖 12 题大模型 Query Rewrite、6 题 Memory 写入/隔离/退出/归因、6 题 AKEP 检索/Citation/Usage/Feedback，以及 6 题模型与两类上下文的组合回答（其中一题验证 AetherGateway DeepSeek）。报告写入 `tmp/triad-experience-latest.json`；测试只使用固定合成 subject 和隔离知识 Space，不读取真实用户记忆，也不输出凭据。可用 `-- --only=Q01,M02,K03,T04` 定点复测。
...
1. AetherGateway `/livez`、`/readyz` 和带鉴权的 `/v1/models`。
2. AetherGateway 管理台中 `deepseek` provider 的“查询余额”；它只读调用 DeepSeek 官方余额接口，充值和账单仍由 DeepSeek 控制台处理。
3. Memory 根 discovery 和 `/readyz`；同时检查 `production_ready`，不要只看 HTTP 200。
4. AKEP `/health`、ContextPack 与 SignalFoundry 配置的 Space；空 Space 即使 HTTP 200 也不会改善回答。
...
7. `curl http://127.0.0.1:3000/api/ready` 与 `npm run doctor`，确认 SignalFoundry 自身数据库和本地归因表。
...
四个仓库必须独立升级和回滚。SignalFoundry 不能导入 AetherGateway、Memory 或 AKEP 内部源码，外部服务不能共享 SignalFoundry 数据库；跨仓兼容只以 `OpenAI-compatible HTTP`、`evolvable-memory-http/v1` 和 `AKEP v0.1 HTTP` 契约为准。
...
api_pgid="$(ps -eo pgid=,cmd= | awk '/uv run signalfoundry-market-api/ {print $1; exit}')"
...
```bash
mkdir -p tmp/runtime
setsid bash -c 'cd services/market-data && exec env SIGNALFOUNDRY_MARKET_HOST=127.0.0.1 SIGNALFOUNDRY_MARKET_PORT=8000 uv run signalfoundry-market-api' > tmp/runtime/market-api.log 2>&1 < /dev/null &
...
生产环境由 `deploy/systemd/signalfoundry-market-maintenance.timer` 在工作日收盘后触发；服务失败必须通过 systemd/journal 监控进入值班告警。`SIGNALFOUNDRY_MARKET_FRESHNESS_MIN_SYMBOLS` 同时约束维护后的覆盖率，不能只用最新日期判断成功。
...
2. `npm run redis:cli` 后执行 `KEYS signalfoundry:*sector*`
