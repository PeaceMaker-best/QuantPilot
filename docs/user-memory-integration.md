...
这篇文档说明如何把独立项目 `evolvable-user-memory` 接入 QuantScope，以及接入后怎样写入偏好、验证聊天是否使用了记忆、提交可归因反馈和排查常见故障。

读完后，你应该能够：

- 启动 Memory 服务并确认它公布了 QuantScope 需要的版本化契约。
- 配置 QuantScope、部署本地归因表并让 readiness 变绿。
- 写入一条全局或项目级偏好，在下一轮聊天中验证 `prepared → 实际归因 → Outcome` 效果。
- 理解“记住偏好”和“改变系统规则”的边界，避免把行情、权限或交易指令写成记忆。

## 当前接入形态

QuantScope 当前把 Memory 作为可选外部服务使用。聊天执行、项目 API 和账号级使用控制已经接通；用户可在 `/account/memory` 明确启用或暂停未来任务的个性化。

| 能力 | 当前入口 | 行为 |
| --- | --- | --- |
| 用户使用控制 | QuantScope `/account/memory` | 未明确启用时不向 Memory 发起任务召回；变更写入本地审计 |
| 显式新增偏好 | 项目对话输入区“记住偏好”/ QuantScope 项目级 Memory API | 用户确认后创建证据、候选和第一条不可变 revision；不自动保存聊天全文 |
| 候选偏好确认 | 用户消息下方候选卡 | 只在高置信稳定表达时提示；交易、授权和一次性要求被过滤；不自动写入 |
| 查看与纠正偏好 | `/account/memory` 或 QuantScope Memory API | 查看当前值、追加修订、保留历史版本 |
| 聊天自动召回 | `/api/chat/:projectId/act` | 每轮执行前召回，并把通过过滤的有界 capsule 交给 PI Agent |
| 使用归因 | Memory `/v1/usages` + QuantScope PostgreSQL `external_memory_uses` | Memory 重建投影并签发 Usage Receipt；QuantScope 只保存 usage/trace/revision 不透明 ID、策略版本和内容哈希 |
| 结果反馈 | 已验证完成消息下方“有帮助 / 不适合我” | UI 先读取 QuantScope 本地归因；持久收据保证重试幂等并拒绝矛盾反馈 |
| 价值透明度 | `/account/memory`“实际价值闭环” | 只统计真正交给 Agent 的 revision 和明确反馈；旧版空归因单独隔离 |
| 可视化管理 | Memory 自带工作台 `http://127.0.0.1:33009` | 用于理解和调试 Memory 自身，不代替 QuantScope 项目授权 |

管理 QuantScope 用户偏好时应调用 QuantScope API，不要让浏览器绕过 QuantScope 直接调用 Memory。这样项目权限、可信用户身份、键白名单和归因账本才不会被跳过。

## 可以不启用 Memory

Memory 是可选组件，不是模型调用、行情读取或 workspace 生成的前置条件。如果当前不需要个性化、还没有部署 Memory，或希望先独立验收 QuantScope 与 ModelPort，只需设置：

```dotenv
QUANTPILOT_MEMORY_ENABLED=0
```

关闭后 QuantScope 不发起 Memory discovery、recall 或 outcome 请求，不需要 `QUANTPILOT_MEMORY_API_URL`、Bearer Token 或 Token Broker。聊天个性化状态为 `disabled`，账号记忆管理、偏好召回和本轮偏好反馈不可用；模型、LLM Query Rewrite、市场数据、工作空间生成、自动验证和预览继续正常工作。
...
下一轮同一用户发起研究任务时，QuantScope 会用当前问题、产品、项目和能力上下文召回记忆。匹配成功并完成二次过滤后，聊天消息 metadata 会先记录“待使用”：
...
`prepared` 只表示生成了候选 capsule，不证明 Agent 已使用。QuantScope 仅在把 capsule 交给 PI Agent 前请求 Memory `/v1/usages`；Memory 从不可变 Trace 重建相同算法和预算的投影，核对源摘要及 revision 子集后签发 `usageId`。QuantScope 随后保存本地归因与[联合上下文清单](context-composition.md)。澄清、拒绝、平台直接生成或用户临时关闭个性化时不会写“已暴露”记录。已验证完成的回复只有查到这份归因后，才显示“本轮实际使用了 N 条个人偏好”及反馈按钮。
...
| 候选偏好真正交给 Agent | QuantScope 写入本轮 revision 归因；最终回复显示反馈入口 |
...
| 项目级偏好用于其他项目 | 被 QuantScope 二次过滤，不进入 Agent |
...
sequenceDiagram
  ...
  participant Q as QuantScope API
  participant M as Memory API
  participant P as QuantScope PostgreSQL
...
- Memory 拥有 Evidence、Belief、RecallTrace、Outcome 和检索策略状态；QuantScope 拥有用户、项目、聊天和授权入口。
- tenant 和 subject 由 QuantScope 服务端生成。生产环境不能把浏览器请求体中的 tenant/subject 当作授权证明。
- 原始记忆正文不写入 QuantScope 聊天 metadata；本地只保留归因所需的不透明 ID 和哈希。
- 记忆 capsule 是不可信偏好数据，不能覆盖当前请求、金融事实、权限、安全策略、工具合同、验证或风险控制。
- `PersonalMemoryControl` 只是 QuantScope 的产品使用围栏，不是法律处理依据，也不代替 Memory 的 ProcessingGrant、抑制与删除编排。治理级删除必须由可信角色通过版本化契约执行，普通账号页面不能借此越权。
...
QuantScope 不根据应用版本号猜测能力，而是读取服务根路径的 `api_contract` 和 `capabilities`：
...
| `QUANTPILOT_MEMORY_TENANT_ID` | QuantScope 这一消费应用独占的稳定 tenant，最长 128 字符；其他产品不得复用 |
...
部署 QuantScope 本地归因表，然后重启 Web 进程使环境变量生效：
...
不要让两个项目共用 PostgreSQL。`ExternalMemoryUse` 不是 Memory 数据副本，只是 QuantScope 对“这一轮到底使用了哪些 revision”的本地审计账本。
...
- QuantScope 日志中没有 `INCOMPATIBLE_CONTRACT`、`NETWORK_ERROR` 或 `TIMEOUT`。

Memory 是 optional 时，`/api/ready` 总体仍可能返回 `ok=true`，所以不能只看顶层状态，必须同时看 `components[].name=memory`。

## 第四步：用户明确启用

登录 QuantScope 后打开 `/account/memory`，点击“启用个性化”。未配置记录默认关闭；聊天会显示 `opted_out`，并且不会向 Memory 发送 query。暂停时在远端 projection 返回后、进入 Agent 前还会再次检查，降低并发撤回期间继续暴露的风险。

当用户说“以后回答时先给结论，再列证据和风险”这类稳定偏好时，QuantScope 会在用户消息下显示候选卡。候选识别完全发生在 QuantScope 本地，只进入消息 metadata；只有用户检查类型、内容和范围并点击“确认并保存”后，才调用版本化 HTTP 契约。关闭候选卡或忽略不会给 Memory 创建负反馈。

关闭只阻止 QuantScope 在未来任务中使用外部记忆，不等于删除远端 Evidence、Revision、Trace 或 Outcome。Memory 删除编排上线前，界面和 API 都不得把关闭描述成删除。

## 第五步：写入并使用第一条偏好

项目 Memory API 使用 QuantScope 自己的项目授权。认证关闭的本地开发环境可以直接使用 `curl`；认证开启时，应先正常登录，再在浏览器控制台运行下面的同源 `fetch` 示例，浏览器会自动携带会话 Cookie。不要为了调试绕过项目权限。

### 1. 显式新增偏好

在已登录的 QuantScope 页面控制台中执行，并替换真实项目 ID：
...
`scope=global` 对该用户的全部 QuantScope 项目生效；`scope=project` 会自动加上当前 `project_id`，只允许同一项目召回。
...
已验证完成的 QuantScope 回复会自动查询本地归因。存在实际暴露的 revision 时，回复下方显示“有帮助 / 不适合我”；点击后分别提交 `helpful` 或 `rejected` Outcome。只有用户明确接受、拒绝、纠正或评价本轮结果后，才提交 Outcome：
...
允许的结果类型是 `helpful`、`accepted`、`harmful`、`rejected` 和 `corrected`。QuantScope 会先查本地归因账本；不属于本轮的 revision 返回 `MEMORY_REVISION_NOT_EXPOSED`，防止把无关结果记到某条偏好上。
...
| QuantScope API | Memory API | 额外保护 |
| --- | --- | --- |
...
- 生产环境应启用 Memory JWT identity，并由可信 token broker 为每个 tenant/subject/purpose 签发短期 token。QuantScope 按 Scope 缓存，不会跨用户复用。
- broker 接收 Basic client authentication 和 `{audience, tenant_id, subject_id, purpose, requested_role:"subject_self"}`，只应在自身策略确认 QuantScope 可代表该 subject 后签发 token；不能无条件相信请求字段。
- broker 返回 `{access_token, token_type:"Bearer", expires_in}`；QuantScope 只接受 60–3600 秒有效期。Memory token 的 `memory_access` 必须使用显式 tenant、subject 和 purpose，禁止 `*`。
- 使用 HTTPS，限制 CORS 和网络入口；浏览器不应持有 QuantScope 的 Memory 服务 token。
...
长期运行由 QuantScope 负责编排，但三个模块保持独立部署：

```text
QuantScope -- OpenAI-compatible HTTP --> ModelPort -- provider route --> Qwen
QuantScope -- OpenAI-compatible HTTP --> ModelPort -- Anthropic protocol --> DeepSeek
QuantScope -- evolvable-memory-http/v1 --> Evolvable User Memory
```

ModelPort 不读取用户记忆，Memory 不调用模型，Qwen 不直接访问两个项目的数据库。QuantScope 先通过 `PersonalMemoryPort` 召回并过滤有界 capsule，再把它作为不可信偏好数据放进 PI Agent prompt；模型 Provider 与 Memory adapter 因而可以独立替换和降级。
...
需要在真实持久化环境完整验证“写入 → 幂等重放 → 同项目召回 → 跨项目隔离 → QuantScope prompt → ModelPort/Qwen 工具调用与续写 → Outcome 幂等重放”时，显式指定两个已经存在的项目：
...
- Qwen 能在 QuantScope 构造的有界 personalization prompt 上完成强制工具调用和工具结果续写。
...
| 新增/列表 API 返回 502 | 网络、超时、无效 JSON或外部契约错误 | 查 QuantScope 日志中的安全错误码和 `x-request-id`，再查 Memory 日志 |
...
| 认证启用后 API 返回 401/403/404 | 未登录、缺少 capability 或没有项目角色 | 按正常 QuantScope 登录和项目授权链路处理，不要直连 Memory 绕过 |
...
| `src/lib/platform/memory/port.ts` | QuantScope 使用的 provider-neutral 端口 |
...
| `src/lib/platform/memory/control.ts` | QuantScope 本地用户使用围栏 |
...
| `src/lib/platform/memory/repository.ts` | QuantScope 本地使用归因账本 |
