...
这篇文档说明如何把独立项目 `evolvable-user-memory` 接入 SignalFoundry，以及接入后怎样写入偏好、验证聊天是否使用了记忆、提交可归因反馈和排查常见故障。

读完后，你应该能够：

- 启动 Memory 服务并确认它公布了 SignalFoundry 需要的版本化契约。
- 配置 SignalFoundry、部署本地归因表并让 readiness 变绿。
- 写入一条全局或项目级偏好，在下一轮聊天中验证 `prepared → 实际归因 → Outcome` 效果。
- 理解“记住偏好”和“改变系统规则”的边界，避免把行情、权限或交易指令写成记忆。

## 当前接入形态

SignalFoundry 当前把 Memory 作为可选外部服务使用。聊天执行、项目 API 和账号级使用控制已经接通；用户可在 `/account/memory` 明确启用或暂停未来任务的个性化。

| 能力 | 当前入口 | 行为 |
| --- | --- | --- |
| 用户使用控制 | SignalFoundry `/account/memory` | 未明确启用时不向 Memory 发起任务召回；变更写入本地审计 |
| 显式新增偏好 | 项目对话输入区“记住偏好”/ SignalFoundry 项目级 Memory API | 用户确认后创建证据、候选和第一条不可变 revision；不自动保存聊天全文 |
| 候选偏好确认 | 用户消息下方候选卡 | 只在高置信稳定表达时提示；交易、授权和一次性要求被过滤；不自动写入 |
| 查看与纠正偏好 | `/account/memory` 或 SignalFoundry Memory API | 查看当前值、追加修订、保留历史版本 |
| 聊天自动召回 | `/api/chat/:projectId/act` | 每轮执行前召回，并把通过过滤的有界 capsule 交给 PI Agent |
| 使用归因 | Memory `/v1/usages` + SignalFoundry PostgreSQL `external_memory_uses` | Memory 重建投影并签发 Usage Receipt；SignalFoundry 只保存 usage/trace/revision 不透明 ID、策略版本和内容哈希 |
| 结果反馈 | 已验证完成消息下方“有帮助 / 不适合我” | UI 先读取 SignalFoundry 本地归因；持久收据保证重试幂等并拒绝矛盾反馈 |
| 价值透明度 | `/account/memory`“实际价值闭环” | 只统计真正交给 Agent 的 revision 和明确反馈；旧版空归因单独隔离 |
| 可视化管理 | Memory 自带工作台 `http://127.0.0.1:33009` | 用于理解和调试 Memory 自身，不代替 SignalFoundry 项目授权 |

管理 SignalFoundry 用户偏好时应调用 SignalFoundry API，不要让浏览器绕过 SignalFoundry 直接调用 Memory。这样项目权限、可信用户身份、键白名单和归因账本才不会被跳过。
...
下一轮同一用户发起研究任务时，SignalFoundry 会用当前问题、产品、项目和能力上下文召回记忆。匹配成功并完成二次过滤后，聊天消息 metadata 会先记录“待使用”：
...
`prepared` 只表示生成了候选 capsule，不证明 Agent 已使用。SignalFoundry 仅在把 capsule 交给 PI Agent 前请求 Memory `/v1/usages`；Memory 从不可变 Trace 重建相同算法和预算的投影，核对源摘要及 revision 子集后签发 `usageId`。SignalFoundry 随后保存本地归因与[联合上下文清单](context-composition.md)。澄清、拒绝、平台直接生成或用户临时关闭个性化时不会写“已暴露”记录。已验证完成的回复只有查到这份归因后，才显示“本轮实际使用了 N 条个人偏好”及反馈按钮。
...
| 候选偏好真正交给 Agent | SignalFoundry 写入本轮 revision 归因；最终回复显示反馈入口 |
...
| 项目级偏好用于其他项目 | 被 SignalFoundry 二次过滤，不进入 Agent |
...
sequenceDiagram
  ...
  participant Q as SignalFoundry API
  participant M as Memory API
  participant P as SignalFoundry PostgreSQL
...
- Memory 拥有 Evidence、Belief、RecallTrace、Outcome 和检索策略状态；SignalFoundry 拥有用户、项目、聊天和授权入口。
- tenant 和 subject 由 SignalFoundry 服务端生成。生产环境不能把浏览器请求体中的 tenant/subject 当作授权证明。
- 原始记忆正文不写入 SignalFoundry 聊天 metadata；本地只保留归因所需的不透明 ID 和哈希。
- 记忆 capsule 是不可信偏好数据，不能覆盖当前请求、金融事实、权限、安全策略、工具合同、验证或风险控制。
- `PersonalMemoryControl` 只是 SignalFoundry 的产品使用围栏，不是法律处理依据，也不代替 Memory 的 ProcessingGrant、抑制与删除编排。治理级删除必须由可信角色通过版本化契约执行，普通账号页面不能借此越权。
...
SignalFoundry 不根据应用版本号猜测能力，而是读取服务根路径的 `api_contract` 和 `capabilities`：
...
不要让两个项目共用 PostgreSQL。`ExternalMemoryUse` 不是 Memory 数据副本，只是 SignalFoundry 对“这一轮到底使用了哪些 revision”的本地审计账本。
...
- SignalFoundry 日志中没有 `INCOMPATIBLE_CONTRACT`、`NETWORK_ERROR` 或 `TIMEOUT`。

Memory 是 optional 时，`/api/ready` 总体仍可能返回 `ok=true`，所以不能只看顶层状态，必须同时看 `components[].name=memory`。

## 第四步：用户明确启用

登录 SignalFoundry 后打开 `/account/memory`，点击“启用个性化”。未配置记录默认关闭；聊天会显示 `opted_out`，并且不会向 Memory 发送 query。暂停时在远端 projection 返回后、进入 Agent 前还会再次检查，降低并发撤回期间继续暴露的风险。

当用户说“以后回答时先给结论，再列证据和风险”这类稳定偏好时，SignalFoundry 会在用户消息下显示候选卡。候选识别完全发生在 SignalFoundry 本地，只进入消息 metadata；只有用户检查类型、内容和范围并点击“确认并保存”后，才调用版本化 HTTP 契约。关闭候选卡或忽略不会给 Memory 创建负反馈。

关闭只阻止 SignalFoundry 在未来任务中使用外部记忆，不等于删除远端 Evidence、Revision、Trace 或 Outcome。Memory 删除编排上线前，界面和 API 都不得把关闭描述成删除。

## 第五步：写入并使用第一条偏好

项目 Memory API 使用 SignalFoundry 自己的项目授权。认证关闭的本地开发环境可以直接使用 `curl`；认证开启时，应先正常登录，再在浏览器控制台运行下面的同源 `fetch` 示例，浏览器会自动携带会话 Cookie。不要为了调试绕过项目权限。

### 1. 显式新增偏好

在已登录的 SignalFoundry 页面控制台中执行，并替换真实项目 ID：
...
`scope=global` 对该用户的全部 SignalFoundry 项目生效；`scope=project` 会自动加上当前 `project_id`，只允许同一项目召回。
...
已验证完成的 SignalFoundry 回复会自动查询本地归因。存在实际暴露的 revision 时，回复下方显示“有帮助 / 不适合我”；点击后分别提交 `helpful` 或 `rejected` Outcome。只有用户明确接受、拒绝、纠正或评价本轮结果后，才提交 Outcome：
...
允许的结果类型是 `helpful`、`accepted`、`harmful`、`rejected` 和 `corrected`。SignalFoundry 会先查本地归因账本；不属于本轮的 revision 返回 `MEMORY_REVISION_NOT_EXPOSED`，防止把无关结果记到某条偏好上。
...
| SignalFoundry API | Memory API | 额外保护 |
| --- | --- | --- |
...
- 生产环境应启用 Memory JWT identity，并由可信 token broker 为每个 tenant/subject/purpose 签发短期 token。SignalFoundry 按 Scope 缓存，不会跨用户复用。
- broker 接收 Basic client authentication 和 `{audience, tenant_id, subject_id, purpose, requested_role:"subject_self"}`，只应在自身策略确认 SignalFoundry 可代表该 subject 后签发 token；不能无条件相信请求字段。
- broker 返回 `{access_token, token_type:"Bearer", expires_in}`；SignalFoundry 只接受 60–3600 秒有效期。Memory token 的 `memory_access` 必须使用显式 tenant、subject 和 purpose，禁止 `*`。
- 使用 HTTPS，限制 CORS 和网络入口；浏览器不应持有 SignalFoundry 的 Memory 服务 token。
...
- Qwen 能在 SignalFoundry 构造的有界 personalization prompt 上完成强制工具调用和工具结果续写。
...
| 认证启用后 API 返回 401/403/404 | 未登录、缺少 capability 或没有项目角色 | 按正常 SignalFoundry 登录和项目授权链路处理，不要直连 Memory 绕过 |
...
| `src/lib/platform/memory/port.ts` | SignalFoundry 使用的 provider-neutral 端口 |
...
| `src/lib/platform/memory/control.ts` | SignalFoundry 本地用户使用围栏 |
...
| `src/lib/platform/memory/repository.ts` | SignalFoundry 本地使用归因账本 |
