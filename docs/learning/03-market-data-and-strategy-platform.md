...
目标：理解 QuantScope 如何管理股票池、ETF/指数池、K 线、估值因子、板块资金、基础组件和缓存。
...
股票池不是“收藏夹”。在 QuantScope 里，股票池是一组可分页、可检索、可计算覆盖率的证券集合。A 股股票池和 ETF/指数池拆开，是为了避免个股策略误把 ETF 或指数当成可买卖个股处理。
