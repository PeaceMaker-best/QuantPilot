...
SignalFoundry 生成的不是一张静态截图，而是一个独立 Next.js 工作空间。每个工作空间都应能独立构建、预览、读取自己的数据文件，并留下可追溯证据。
...
在量化场景里，一个漂亮页面如果没有数据来源和字段口径，基本没有研究价值。SignalFoundry 把最终数据和证据拆开，是为了让页面和验证器分别回答两个问题：
...
```text
为 SignalFoundry 生成一张横向流程图，风格干净、专业、适合技术文档。
