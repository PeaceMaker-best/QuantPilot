...
本文是 SignalFoundry 本地数据保留、测试隔离、备份和清理的权威入口。删除前先判断数据所有权；
...
5. Memory 用 `(tenant_id, subject_id)` Scope erasure；AKEP 用 revoke/erase 生命周期；AetherGateway 的
   append-only 预算事件不得从 SignalFoundry 侧删除。
...
- 不把 Memory、AKEP 或 AetherGateway 的测试清理扩大到其他 tenant/project/environment。
