...
QuantScope 的唯一 Agent 执行框架是开源
[earendil-works/pi](https://github.com/earendil-works/pi)。PI Agent 负责模型与工具之间的
完整多轮循环；QuantScope 负责产品权限、副作用治理、持久化、并发、预算和交付验收。
...
HTTP / Generation Worker
  ...
  -> QuantScope Skills、Mission 与类型化工具
  -> PiAgentRunEngine
       -> QuantScope messages/context -> PI `AgentContext`
       -> runAgentLoopContinue(...)
            -> QuantScope Provider adapter -> PI StreamFn
...
Provider adapter 把 QuantScope 的 ModelPort、本地 Qwen、受控 DeepSeek 和确定性测试
...
| QuantScope Agent 治理 | Context Manager、预算、收敛、工具允许列表、审批、operation ID |
...
PI 不提供内建权限系统。能够被 PI 调用的工具仍必须通过 QuantScope 的 schema、路径、
