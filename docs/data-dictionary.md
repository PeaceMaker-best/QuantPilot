...
这份文档记录 SignalFoundry 当前最重要的数据表、字段口径和使用边界。它的目标不是替代 SQL，而是让前端、后端、skills 和评测在同一套事实上工作。
...
| 短期缓存 | Redis | `signalfoundry:*` | 板块资金、行情摘要、接口短 TTL，不作为事实库 |
...
| `factor_value` | 数值型结果 |
| `provider` | `signalfoundry`、`baostock`、`eastmoney` 等 |
...
| `source` | `eastmoney`、`manual`、`signalfoundry` |
| `tags` | 分组标签 |
| `metadata` | 池规则、统计摘要 |
